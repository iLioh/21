...
function createCenterTextures() {
  ...
  bgGrad.addColorStop(0.7, "#653a1e");
  ...
  for (let i = 1; i <= dotCount; i++) {
    ...
    bumpGrad.addColorStop(0.6, "#b8b8b8");
...
export function FlowerField({ timeline, count }: FlowerFieldProps) {
  ...
  const flowers = useMemo<FlowerDatum[]>(() => {
    ...
    while (list.length < targetCount && attempts < maxAttempts) {
      ...
      const groundY =
        -0.7 + Math.sin(x * 0.3) * 0.08 + Math.cos(z * 0.25) * 0.06;
      const height =
        1.32 + random() * 0.68 + (z > 2.0 ? (random() - 0.5) * 0.25 : 0);
      ...
      const rotX = -0.1 + (random() - 0.5) * 0.08;
      const rotY = (x > 0 ? -0.09 : 0.09) + (random() - 0.5) * 0.1;
  ...
  useLayoutEffect(() => {
    ...
    flowers.forEach((flower, flowerIndex) => {
      ...
      dummy.position.set(
        flower.x,
        flower.groundY + flower.height * 0.5,
        flower.z,
      );
      ...
      for (let petalIndex = 0; petalIndex < PETALS; petalIndex += 1) {
        ...
        const angle =
          (petalIndex / PETALS) * Math.PI * 2 + (flowerIndex % 4) * 0.03;
    ...
    grassTufts.forEach((tuft, index) => {
      dummy.position.set(tuft.x, -0.7 + tuft.height * 0.48, tuft.z);
  ...
  return (
    <group ref={group}>
      ...
      <instancedMesh
        ref={petals}
        args={[undefined, undefined, flowers.length * PETALS]}
        castShadow
      >
      ...
      <mesh position={[0, -0.7, -3.5]} rotation={[-Math.PI / 2, 0, 0]}>

...
const earthFragment = `
  ...
  void main() {
    ...
    vec3 ocean = mix(vec3(.014,.06,.15), vec3(.03,.24,.38), max(0.0,n.y));
    vec3 earth = mix(ocean, vec3(.10,.25,.12), land);
    float clouds = smoothstep(.63,.78,noise(n*11.0 + vec3(uTime*.015,0.,0.)));
    earth = mix(earth, vec3(.85,.90,.92), clouds*.40);
    float light = max(.22, dot(normalize(vNormal), normalize(vec3(-.4,.8,.9))));
    float city = step(.78, noise(n*26.0)) * (1.0-land*.45) * smoothstep(.1,.65,-n.y+.3);
    earth = earth * (.28 + light*.88) + vec3(1.0,.58,.15)*city*1.2;
...
const atmosphereFragment = `
  ...
  void main(){
    ...
    gl_FragColor=vec4(vec3(.18,.54,1.0)*rim*2.2, rim*uOpacity);
  }
`
...
export function EarthDeparture({ timeline }: { timeline: React.RefObject<TimelineState> }) {
  ...
  useFrame(({ clock }) => {
    ...
    const emerge = smoothstep(0.08, 0.16, p)
    const fade = emerge * (1 - smoothstep(0.38, 0.50, p))
    if (group.current) {
    ...
    if (atmosphere.current) atmosphere.current.uniforms.uOpacity.value = fade * 0.95
  })
